...
def rule(df):
    ...
    call_take_100 = df.groupby('phone_no_m')['opposite_no_m'].nunique().reset_index(name='voc_nunique')
    ...
    # 拨出100个电话以上
    call_100 = df[df['calltype_id'] == 1].groupby('phone_no_m')['opposite_no_m'].nunique().reset_index(name='voc_nunique')
    call_100_phone = call_100[call_100['voc_nunique'] > 100]['phone_no_m'].to_list()
    print('拨出100个电话以上', len(call_100_phone))

    # 回拨率
    phone_id_list = [num2id[x] for x in df['phone_no_m'].drop_duplicates().tolist()]
    ...
    # 手机换卡次数大于10
    phone_imei_count = df.groupby('phone_no_m')['imei_m'].nunique().reset_index(name='imei_phone')
